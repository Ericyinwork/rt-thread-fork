...
/* ADC采集类型 */
#define RSCDRRM020NDSE3_ADC_TEMPERATURE 0x04 // 温度
#define RSCDRRM020NDSE3_ADC_PRESSURE 0x06 // 压力
...
static rt_err_t rscdrrm020ndse3_spi_send_adc_cmd(uint8_t cmd)
{
    ...
    return rt_spi_send(spi_device, &cmd, 1);
}
...
static rt_err_t rscdrrm020ndse3_start(rt_device_t dev)
{
	...
	*/
	{ // 消除编译警告
		uint8_t mode = RSCDRRM020NDSE3_ADC_TEMPERATURE | RSCDRRM020NDSE3_ADC_330HZ | RSCDRRM020NDSE3_ADC_NORMAL;
    ...
    while (1)
	{
		...
		{// 数据已就绪
			...
			if (RSCDRRM020NDSE3_PRESSURE == rscdrrm020ndse3->mode) // pressure
			{
				...
				send_buf[0] = 0x40 | ((0x01 & 0x03) << 2) | ((1 - 1) & 0x03); // WREG command [0100 RRNN],
				send_buf[1] = RSCDRRM020NDSE3_ADC_TEMPERATURE | RSCDRRM020NDSE3_ADC_330HZ | RSCDRRM020NDSE3_ADC_NORMAL; // mode
				send_buf[2] = 0x08; // Send 08h command to start data conversion on ADC.
				rt_size_t trans_len = rt_spi_transfer(rscdrrm020ndse3_dev.spi_adc_device, send_buf, recv_buf, 3);
				if (3 == trans_len)
				{
					/* 
						处理压力数据
					...
					*/
					pressure = (uint32_t)recv_buf[0] << 16 | (uint32_t)recv_buf[1] << 8 | (uint32_t)recv_buf[2];
			...
			else //if (RSCDRRM020NDSE3_TEMPERATURE == rscdrrm020ndse3->mode) // temperature
			{
				...
				uint8_t recv_buf[3] = {0};
				send_buf[0] = 0x40 | ((0x01 & 0x03) << 2) | ((1 - 1) & 0x03); // WREG command [0100 RRNN],
				send_buf[1] = RSCDRRM020NDSE3_ADC_PRESSURE | RSCDRRM020NDSE3_ADC_330HZ | RSCDRRM020NDSE3_ADC_NORMAL; // mode
				send_buf[2] = 0x08; // Send 08h command to start data conversion on ADC.
				rt_size_t trans_len = rt_spi_transfer(rscdrrm020ndse3_dev.spi_adc_device, send_buf, recv_buf, 3);
				if (3 == trans_len)
				{
					...
					*/
					temperature = ((uint32_t)recv_buf[0] << 6) | ((uint32_t)recv_buf[1] >> 2);
				}
			}
		}
	}
...
static rt_err_t rscdrrm020ndse3_init(rt_device_t dev)
{
	...
	*/
	...
	rscdrrm020ndse3->adc_cfg_param[2] = eeprom_buf[64];
